...
#include <String.h>
int vt_start_ssid = 0, vt_start_pass = 50, vt_start_blynk = 100, ssid_size, pass_size, blynk_size;
int vt_size_ssid = 301, vt_size_pass = 302, vt_size_blynk = 303;
String get_str(int start_, int stop_) {
  String data;
...
int add_str(int start_, int stop_, String nd) {
  for (int i = start_; i <= stop_; i++) {
    int stt = i - start_;
...
void config_eeprom(String token) {
  String blynk_auth;
  int size_blynk;
  blynk_auth = token;
  size_blynk = blynk_auth.length();
  add_str(vt_start_blynk, vt_start_blynk + size_blynk - 1, blynk_auth);
  EEPROM.write(vt_size_blynk, size_blynk);
  EEPROM.commit();
}
...
void WiFiManager::handleRoot() {
  ...
  page += _apName;
  page += String(F("</h1>"));
  page += String(F("<h4>Mobile Robot</h4>"));
...
void WiFiManager::handleWifiSave() {
  ...
  _token = server->arg("tk").c_str();
  
  config_eeprom(String(_token));
  //parameters
  for (int i = 0; i < _paramsCount; i++) {
    if (_params[i] == NULL) {
      break;
